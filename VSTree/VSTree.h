...
public:
	...
	/* retrieve candidate result set by the var_sig in the _query. */
    void retrieve(SPARQLquery& _query, string& internal_vertices_arr);
private:
	int root_file_line;
	int node_num;
	int entry_num;
	...
	/* retrieve the candidate entity ID which signature can cover the_entity_bit_set, and add them to the  _p_id_list. */
	void retrieveEntity(const EntityBitSet& _entity_bit_set, IDList* _p_id_list, string& internal_tag_arr);
