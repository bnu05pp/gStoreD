...
Join::join_pe(BasicQuery* _basic_query, string &internal_tag_str)
{
    ...
    bool ret1 = this->filter_before_join();
    //printf("this->filter_before_join(internal_tag_str);\n");
    if(!ret1)
    {
        this->clear();
        return false;
    }
	//printf("this->add_literal_candidate(internal_tag_str);\n");
    this->add_literal_candidate();
...
Join::join(string &internal_tag_str)
{
	...
	for(int j = 0; j < this->var_num; ++j){
		...
		//printf("variable %d has %d candidates and %d internal candidates and count(63811) = %d.\n", j, cur_size, cur_can_set.size(), cur_can_set.count(63811));
...
Join::multi_join(string &internal_tag_str, vector< set<int> >& can_set_list)
{
	...
	for(int _start_idx = 0; _start_idx < this->var_num; _start_idx++){
		...
		//printf("_start_idx %s has id %d and current_table.size() ================= %d\n", this->basic_query->getVarName(_start_idx).c_str(), _start_idx, this->current_table.size());
		...
		while(!this->mystack.empty() && this->current_table.size() != 0)
		{
			...
			bool flag = false;
			//printf("before new_join_with_multi_vars_not_prepared, dealed_id.size() = %d and id2 = %d and can_list_size = %d and can_list.size() = %d and edges.size() = %d\n", dealed_id.size(), id2, can_list_size, can_list.size(), edges.size());
	...
	for(TableIterator it0 = this->current_table.begin(); it0 != this->current_table.end(); ++it0)
	{
		...
		*/
		...
		for(int id_idx = 0; id_idx < dealed_id.size(); id_idx++){
			...
			*/
			if(id_list_len == 0 && (ele >= Util::LITERAL_FIRST_ID || internal_tag_str.at(ele) == '1'))
			{
				//id_list == NULL in this case, no need to free
				matched = false;
				continue;
...
Join::add_literal_candidate()
{
    ...
    for(int i = 0; i < this->var_num; i++)
    {
		...
		//printf("isSatelliteInJoin ");

		// for these literal variable, we still consider them
		/*
		...
		*/
		...
		//printf(" and has %d\n", after_add_literal_candidate_list_len);
