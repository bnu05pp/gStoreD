...
bool
Strategy::handleCandidate(SPARQLquery& _query, string &internal_tag_str, vector< vector<int> >& candidates_vec, vector< vector<int> >& candidates_id_vec, vector< vector<int> > &_query_dir_ad, vector< vector<int> > &_query_pre_ad, vector< vector<int> > &_query_ad, set<int> &satellites_set, ResultFilter* _result_filter)
{
#ifdef MULTI_INDEX
	...
	for (; iter != queryList.end(); iter++){
		if ((**iter).getEncodeBasicQueryResult())
		{
			...
			{
			case 0:
				this->handler0_1(*iter, candidates_vec, internal_tag_str, candidates_id_vec, _result_filter);
				break;
			case 1:
			...
			//stringstream query_ad_ss;
			map<int, int> varID2PosMap;
			int var_pos = 0;
			for(int var_id = 0; var_id < varNum; var_id++){
				if((*iter)->getVarDegree(var_id) == 0){
					continue;
				}
				varID2PosMap.insert(make_pair(var_id, var_pos));
				//query_ad_ss << var_id << " maps to var_pos = " << var_pos << " and " << (*iter)->getVarName(var_id) << endl;
				var_pos++;
			}
			
			for(int var_id = 0; var_id < varNum; var_id++){
				int var_degree = (*iter)->getVarDegree(var_id);
				if(var_degree == 0){
					continue;
				}
				var_pos = varID2PosMap[var_id];
				//query_ad_ss << "var_pos = " << var_pos << " : ";
				
				_query_ad.push_back(vector<int>());
				_query_pre_ad.push_back(vector<int>());
				_query_dir_ad.push_back(vector<int>());
				
				if(var_degree == 1){
					satellites_set.insert(var_pos);
				}
				
				for (int k = 0; k < var_degree; k++){
					int var_id2 = (*iter)->getEdgeNeighborID(var_id, k);
					if(var_id2 == -1)
						continue;
					
					int var_pos2 = varID2PosMap[var_id2];
					
					int pre_id = (*iter)->getEdgePreID(var_id, k);
					int edge_type = (*iter)->getEdgeType(var_id, k);
					
					_query_ad[var_pos].push_back(var_pos2);
					_query_pre_ad[var_pos].push_back(pre_id);
					if (edge_type == Util::EDGE_OUT)
					{
						_query_dir_ad[var_pos].push_back(0);
					}
					else
					{
						_query_dir_ad[var_pos].push_back(1);
					}
					
					//query_ad_ss << _query_ad[var_pos][_query_ad[var_pos].size() - 1] << "-" << _query_pre_ad[var_pos][_query_ad[var_pos].size() - 1] << "-" << _query_dir_ad[var_pos][_query_ad[var_pos].size() - 1] << "\t";
				}
				//query_ad_ss << endl;
			}
			
			//cout << query_ad_ss.str().c_str();
		}
	}
#else
...
bool
Strategy::handle(SPARQLquery& _query, string &internal_tag_str, ResultFilter* _result_filter)
{
#ifdef MULTI_INDEX
	Util::logging("IN GeneralEvaluation::handle");
	...
	if ((**iter).getEncodeBasicQueryResult())
	{
		...
		{
		case 0:
			this->handler0_0(*iter, internal_tag_str, result_list, _result_filter);
			break;
		case 1:
			this->handler1(*iter, result_list);
			break;
...
void
Strategy::handler0_0(BasicQuery* _bq, string& internal_tag_str, vector<int*>& _result_list, ResultFilter* _result_filter)
{
	int star_flag = 0;
	if(star_flag == 0){
		_bq->setRetrievalTag();
...
void
Strategy::handler0_1(BasicQuery* _bq, vector< vector<int> >& candidates_vec, string& internal_tag_str, vector< vector<int> >& candidates_id_vec, ResultFilter* _result_filter)
{
	int star_flag = 0;
	if(star_flag == 0){
		_bq->setRetrievalTag();
	...
	Join *join = new Join(kvstore);
	join->join_can(_bq, internal_tag_str, candidates_vec, candidates_id_vec);
