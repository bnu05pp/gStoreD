...
public:
	...
	bool load();
	//bool load(string& db_folder, int myRank);
	bool unload();
	bool query(const string _query, ResultSet& _result_set, FILE* _fp, int myRank);
	SPARQLquery query(const string _query, ResultSet& _result_set, string& _res, int myRank);
	int queryASK(const string _query, ResultSet& _result_set, string& _res, int myRank);//, string& query_graph);
